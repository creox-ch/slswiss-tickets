...
- ⚠ **`MARKET_ADMIN_EMAILS` не содержит адресов Иванны.** Проверено на проде через сессию продавца: `/market/admin` → 404 для `amossokn@gmail.com` и `ivannasm@gmail.com`, 200 — для `assistant@creox.ch`. Пока адрес Иванны не добавлен в переменную (Production + Preview) с передеплоем, принять вещь первого живого продавца она не сможет. Это блокирует заведение клиентов, а не только тесты.
