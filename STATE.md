...
**Билеты продаются ещё и мимо нашего checkout — и билет по ним не выдаётся.**
Иванна завела платёжную страницу Payrexx (`slswiss.payrexx.com/pay?tid=9b3ce670`)
ради промокодов на стороне провайдера: свой промо-код снят с проекта 04.09 и лежит
в карантине. У такой оплаты нет нашего `reference_id`, поэтому вебхук отбрасывает
её первой же проверкой (`route.js:55`), молча и с ответом 200. Следствия: нет строки
в `tickets`, нет `qr_token` — **на входе сканировать нечего**, нет номера FP-2026-NNNN,
нет письма, покупатель не попадает в счёт мест. Это ровно тот риск, что записан ниже
в «Закрытых решениях» как «Gateway API, не статический Paylink»; решение пересмотрено
по внешней причине, и запись теперь расходится с жизнью.
**Пока оплат по ней не было.** Выдаём вручную: инструмент лежит вне git —
`Downloads/creox/_tools/manual-ticket/` (README там же). Он печатает `insert` в
`tickets` + вызов `assign_forum_ticket_no`, дальше SQL через Supabase и письмо через
Resend нашим же шаблоном — билет получается настоящий, с рабочим QR.

**Коды на странице Payrexx заведены** (05.09): `SVOI10` −10%, `DRUG20` −20%,
`PROMO30` −30%, `PARTNER50` −50%, `BARTER99` −99%; лимит 50 использований у
каждого, у `BARTER99` — 10. Считает их сам Payrexx: наш вебхук этих оплат не
видит. Скидка идёт поверх Early Bird −25%, который уже в ценах страницы, и
режет заказ целиком — исключить позицию Payrexx не умеет, и 100% скидку не даёт
вовсе (отсюда 99). Поэтому **ланч со страницы убран**: обед — живые деньги
кейтерингу (35 CHF за день, 70 за два), и по `BARTER99` мы платили бы за него
сами. На сайте ланч остаётся. Суммы для сверки при ручной выдаче — в памяти
агента, запись `payrexx-paylink-tickets`.

**CI больше не гоняет сборку и тесты на правке одних `.md`** (PR #62). Оба job
запускаются всегда и отчитываются зелёным — иначе обязательная проверка
зависает в «Expected», как 02.09 при `paths-ignore`, — но тяжёлые шаги
пропускают. gitleaks не пропускается никогда: ключ можно вписать и в текст.
Вывести `.md` из-под требования PR нельзя в принципе: ни classic protection, ни
rulesets не умеют исключений по путям.

**Локальный гейт `npm run precheck`** (PR #60). Проверок на GitHub сейчас не
происходит ни разу: все аккаунты заблокированы (последний — 03.09), а оба workflow
триггерятся только на `pull_request` и push в `main` — push в ветку не запускает
ничего. Гейт повторяет CI на машине за 90 с: секреты, сверка `package-lock` с
`package.json`, `audit:gate`, сборка, все тесты. Падает на первом красном. Проверен
подставными поломками (ключ вида `re_…` и разошедшийся lock — оба ловятся); найденный
секрет не печатается, только файл и строка. **Не заменяет gitleaks**: тот сканирует
всю историю репозитория, а гейт — текущее состояние файлов. gitleaks локально не
установлен.

**Схема зала в коде догнала сайт** (PR #59): VIP ряды 1–3 · 45 мест, Premium 4–8 · 75,
Standard 9–20 · 180 (сумма 300 = зал). Стояло 1–2 · 30 / 3–5 · 45 / 6–20 · 225 —
прежняя схема. В письмо и описание заказа эти поля не попадают, покупатель расхождения
не видел, но это заготовка под уведомление о заполнении категории из «Фазы C».
⚠ Лимит мест по-прежнему **никто не проверяет**: продать больше, чем есть, можно и
через сайт, и через страницу Payrexx.

**Доступ к GitHub: push работает деплой-ключами.** Аккаунтов нет ни одного, PR открыть
некому — оба сегодняшних PR мержила Ксения (`maincreox`). Ключи заведены на семь репо;
push в `main` проходит везде, кроме этого репозитория (здесь защита требует PR).
Подробности и карта — в памяти агента, запись `github-accounts-suspended`.

### ✅ Живая оплата в боевом режиме ПРОШЛА (2026-08-27, снято из «Срочного» 02.09)

Висевший с 17.08 пункт «боевая оплата ни разу не проходила целиком» закрыт: **27.08 прошла оплата в боевом Payrexx** (сверено с базой 02.09). Билет `frankenplatz-2026-10` на 239 CHF, `ticket_no` 1, `paid` через 2 минуты после создания, `payrexx_tx_id` записан — вебхук в боевом режиме отработал штатно на Next 15 + resend 6. Покупка настоящая (не возвращена), покупатель — реальный человек. Иванна подтвердила прохождение 02.09. Не проверено этой покупкой: чек-ин по QR на входе (`checked_in_at` пуст — и не должен быть до события) и формально содержимое письма (QR картинкой + вложение `ticket-qr.png`) — жалоб не поступало.
...
- ⚠ **Тест, читающий исходник, — не проверка.** Ищут подстроку в файле, а не поведение: краснеют на переименование, молчат на сломанную логику. Найдено независимой рецензией. **Уточнено 21.08: таких тестов 20, а не 11, и живут они в четырёх файлах** — `market-turnkey.spec.js` (11 из 13), `market-empty-states.spec.js` (3 из 3), `email-attachment-schema.spec.js` (3 из 3), `market-items.spec.js` (3 из 54, строки 409/436/449). Пока они не переписаны, общее число зелёных тестов завышает покрытие — называя его, вычитать эти двадцать.
...
- **Фаза C билетов:** отчёт по заказам (`v_forum_orders` — кто что купил, сколько ланчей по дням для кейтеринга) и уведомление при заполнении категории. Мест по категориям: **VIP 45 / Premium 75 / Standard 180** (сумма 300 = зал). Цифры выправлены 05.09 по сайту и странице оплаты — в коде до этого стояла прежняя схема зала (30/45/225), она нигде не читалась. Лимит по-прежнему **никто не проверяет**: продать больше мест, чем есть, можно и через сайт.
