...
- **GitHub — без аккаунта, через deploy key (с 2026-09-04).** Заблокированы все три учётки, которыми работали (`assistant-web` и `amossokn-oss` 20.08, `ivannasm80` 03.09; причины неизвестны). `gh` есть, но не авторизован; локальные токены вычищены. Push в этот репо идёт SSH-ключом, привязанным к репозиторию: `~/.ssh/tickets_deploy`, псевдоним `github-tickets` в `~/.ssh/config`, remote `git@github-tickets:creox-ch/slswiss-tickets.git`. Проверено делом 04.09 (fetch + push ветки). **PR через deploy key открыть нельзя** — ветку пушит агент, PR открывает владелец организации по ссылке из вывода push. Темп по-прежнему спокойный: по одному коммиту и ветке, без пачек.
...
- **2026-09-04** — продукт не менялся; день про доступ к GitHub. (1) Третья учётка `ivannasm80` заблокирована 03.09 → все локальные токены GitHub удалены, push переведён на **deploy keys** (этот репо + frankenplatz, handbook, motozuerich.ch, портал MZ), см. «Ограничения». (2) **Промокоды сняты с проекта решением Иванны**: все три блокировки легли по датам на заливку именно этой работы (20.08 и 03.09), причина не выяснена, связь — совпадение дат, не доказательство. Удалены ветки `feat/promo-codes`/`feat/promo` на GitHub и локально, ветки `feat/promo-field`/`feat/promo-input` в frankenplatz, а в боевой БД миграцией `remove_promo_codes` — таблица `promo_codes` (была пустая), функция `count_promo_uses`, индексы и столбцы `tickets.paid_amount`/`amount_mismatch` (были сплошь NULL; 14 билетов целы). В `main` промокодов никогда не было. Копия работы — локальный карантин `Downloads\creox\_quarantine\promo-2026-09-04` (bundle + patch + sql), не в git. Правило: не поднимать без явного решения Иванны. (3) `gh`-зависимые пункты этого файла (ревью PR, «gh авторизован») устарели — PR открывает человек.
