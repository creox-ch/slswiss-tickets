...
**Живая оплата в боевом режиме ни разу не проходила целиком.** В таблице `tickets` единственная строка `paid` — от 2026-07-30, ещё из тестового режима; две записи от 2026-08-07 висят в `pending`. Скорее всего просто никто не покупал (трафика на сайт нет), но вся цепочка после оплаты — вебхук → `paid` → письмо с QR → чек-ин — в боевом режиме не отрабатывала. Проверяется одной реальной покупкой с возвратом; оплату проводит Иванна.
...
8. 🔴 **Апгрейд Next 14.2.35 → 15.5.21+.** Первый же полный аудит показал: за веткой 14.x числится **21 advisory уровня high**, и она их больше не получает — все закрыты в 15.5.21+, `npm audit fix` предлагает 16.3.1 (мажор). Эксплуатируемость у нас оценена как низкая (нет middleware, rewrites, Server Actions, next/image, CSP-nonce), поэтому принято до **2026-10-01** — дата не с потолка: форум 24–25.10, к нему пойдёт трафик. После этой даты CI начнёт падать сам. Апгрейд делать отдельно от всего остального: на этом же приложении висят оплата, вебхук и письма.
