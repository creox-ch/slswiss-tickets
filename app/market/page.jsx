  ...
  return (
    <Shell>
      ...
      <section style={S.card}>
        ...
        {/* Список обещает только то, что кабинет уже делает. Раньше здесь стояло
            «люди бронируют — ты видишь спрос здесь» и «после продажи отмечаешь
            проданной»: ни брони, ни отметки продажи в кабинете нет, и продавец,
            только что заплативший за пакет, не находил ни того ни другого. */}
        <ul style={S.list}>
          <li>Мы проверяем описание и ставим карточку в каталог.</li>
          <li>
            Когда покупатель напишет по твоей вещи, мы пересылаем его письмо тебе — отвечаешь
            прямо из почты.
          </li>
          <li>На маркете 27 сентября вещь получает QR-этикетку с твоей витриной.</li>
        </ul>
        <p style={S.hint}>
          Деньги за вещь покупатель отдаёт тебе напрямую — мы платежи не принимаем. После маркета
          мы сверим с тобой состоявшиеся сделки и выставим счёт на комиссию 12%.
        </p>
      </section>
    </Shell>
  );
