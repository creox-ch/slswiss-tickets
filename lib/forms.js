...
/**
 * HTML письма-БИЛЕТА форума (после оплаты). Тот же премиум-шаблон дизайнера
 * (emailShell + карточка reportRowsDesigned), что и у писем калькуляторов —
 * единый стиль. Детали заказа (что куплено, сумма) и QR на входе.
 * qrUrl — картинка QR (её строит sendForumTicketEmail из lib/ticket).
 */
export function renderForumTicketHtml(sub, qrUrl, qrToken) {
  const s = plainObject(sub);
  ...
  const qrSafe = escapeHtml(String(qrUrl || ''));
  const token = escapeHtml(String(qrToken || ''));
  const qrBlock = `
  <tr><td class="fp-pad" align="center" style="padding:22px 34px 4px">
    <img src="${qrSafe}" alt="QR билета" width="220" height="220" style="width:220px;height:220px;border-radius:14px;background:#ffffff;padding:12px" />
    <p style="margin:12px 0 0;font-family:'Manrope',Arial,Helvetica,sans-serif;font-size:12px;color:#7A6C93">Код билета: ${token}</p>
  </td></tr>`;
  return emailShell({
    title: 'Frankenplatz — твой билет',
    heading: `Билет у тебя${name ? ', ' + name : ''} 🎟️`,
    lead: 'Оплата прошла. Покажи этот QR на входе — его отсканируют. Билет также лежит вложением в письме.',
    blocks: rows + qrBlock,
    footerNoteHtml:
      'Ты получил(а) это письмо, потому что купил(а) билет на frankenplatz.ch. Вопросы — <a href="mailto:info@frankenplatz.ch" style="color:#7A6C93">info@frankenplatz.ch</a>.',
  });
}

/** Plain-text версия письма-билета (text-часть рядом с HTML — анти-спам). */
export function renderForumTicketText(sub, qrToken) {
  const s = plainObject(sub);
