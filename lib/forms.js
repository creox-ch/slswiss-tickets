...
export function renderRegistrationHtml(sub) {
  ...
  return `
    <div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;color:#1a1a1a">
      ...
      <h2 style="margin:0 0 12px;font-size:22px;color:#231433">Ты в списке ранней регистрации</h2>
      <p style="margin:0 0 16px;font-size:14px;line-height:1.5;color:#444">
        ${hi} Мы записали тебя. <b>Это ещё не билет</b> — продажи пока не открыты.
        Как только появятся цены и схема билетов, ты узнаешь об этом первым(ой),
        до публичного анонса.
      </p>
      ${rows ? `<table style="border-collapse:collapse;font-size:14px;width:100%">${rows}</table>` : ''}
      <p style="margin:18px 0 0;font-size:14px;line-height:1.5">
        Пока можно посчитать свои деньги в
        <a href="${escapeHtml(FORUM_URL)}/calculators/" style="color:#7B3FE4;font-weight:bold">калькуляторах форума</a>
        — бесплатно и без регистрации.
      </p>
      <p style="margin:16px 0 0;font-size:11.5px;color:#999">
        Ты получил(а) это письмо, потому что оставил(а) заявку на
        <a href="${escapeHtml(FORUM_URL)}" style="color:#999">frankenplatz.ch</a>.
      </p>
    </div>
  `;
}
...
export function renderRegistrationText(sub) {
  ...
  return [
    ...
    ...(rows ? ['', rows] : []),
    '',
    `Пока можно посчитать свои деньги в калькуляторах форума — бесплатно и без регистрации: ${FORUM_URL}/calculators/`,
    '',
    '—',
...
export function renderSpeakerHtml(sub) {
  ...
  return `
    <div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;color:#1a1a1a">
      ...
      <h2 style="margin:0 0 12px;font-size:22px;color:#231433">Анкета спикера получена</h2>
      <p style="margin:0 0 16px;font-size:14px;line-height:1.5;color:#444">
        ${hi} Мы получили твою заявку и обязательно её посмотрим.
        <b>Ответим в течение недели</b> — независимо от решения, без ответа не оставим.
      </p>
      ${rows ? `<p style="margin:0 0 6px;font-size:13px;color:#888">Что ты отправил(а):</p>
      ...
      <p style="margin:16px 0 0;font-size:11.5px;color:#999">
        Ты получил(а) это письмо, потому что оставил(а) анкету спикера на
        <a href="${escapeHtml(FORUM_URL)}/speakers.html" style="color:#999">frankenplatz.ch</a>.
      </p>
    </div>
  `;
}
...
export function renderReportHtml(sub) {
  ...
  const forum = escapeHtml(FORUM_URL);
...
<table role="presentation" class="fp-wrap" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px;max-width:600px;background:#140A1F;border-radius:20px;overflow:hidden;border:1px solid rgba(255,255,255,.08);">
  ...
  <tr><td class="fp-pad" style="padding:38px 34px 6px">
    ...
    <h1 style="margin:0;font-family:'Unbounded','Manrope',Arial,Helvetica,sans-serif;font-weight:800;font-size:30px;line-height:1.18;letter-spacing:-.02em;color:#FFFFFF">Твой расчёт — <span style="background:linear-gradient(90deg,#B98BFF 0%,#F5C969 100%);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;color:#F5C969">${what}</span></h1>
    <p style="margin:16px 0 0;font-family:'Manrope',Arial,Helvetica,sans-serif;font-size:15px;line-height:1.6;color:#C3B7D4">Ты сделал(а) расчёт на сайте форума и попросил(а) прислать результат. Вот он:</p>
  </td></tr>
${rowsCard}
  ...
  <tr><td class="fp-pad" align="center" style="padding:24px 34px 30px">
    <p style="margin:0 0 18px;font-family:'Manrope',Arial,Helvetica,sans-serif;font-size:15px;line-height:1.6;color:#C3B7D4">Полные разборы с экспертами — на форуме <span style="color:#B98BFF;font-weight:700">Frankenplatz</span>,<br>24–25 октября 2026.</p>
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center" style="margin:0 auto"><tr>
      <td bgcolor="#F5C969" style="border-radius:40px;background:#F5C969;background:linear-gradient(180deg,#F7D488 0%,#E6B450 100%)">
        <a href="${forum}" style="display:inline-block;padding:15px 34px;font-family:'Manrope',Arial,Helvetica,sans-serif;font-weight:800;font-size:15px;color:#2A1A05;text-decoration:none;border-radius:40px">Забронировать место</a>
      </td>
    </tr></table>
  </td></tr>

  <tr><td class="fp-pad" style="padding:24px 34px 28px;border-top:1px solid rgba(255,255,255,.08);background:#100818">
    <div style="font-family:'Manrope',Arial,Helvetica,sans-serif;font-size:12px;color:#7A6C93;line-height:1.5">Ты получил(а) это письмо, потому что оставил(а) свой e-mail в калькуляторе на сайте форума.</div>
    <a href="mailto:info@frankenplatz.ch?subject=Unsubscribe" style="font-family:'Manrope',Arial,Helvetica,sans-serif;font-size:12px;color:#7A6C93;text-decoration:underline;display:inline-block;margin-top:8px">Отписаться</a>
  </td></tr>
</table>
</td></tr></table>
</body>
...
export function renderReportText(sub) {
  ...
  return [
    ...
    '',
    `Полные разборы с экспертами — на форуме Frankenplatz, 24–25 октября 2026: ${FORUM_URL}`,
    '',
    '—',
...
export function renderConfirmHtml(sub, confirmUrl) {
  ...
  return `
    <div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;color:#1a1a1a">
      ...
      <h2 style="margin:0 0 12px;font-size:22px;color:#231433">Подтверди подписку</h2>
      <p style="margin:0 0 18px;font-size:14px;line-height:1.5;color:#444">
        ${hi} Остался один шаг. Нажми кнопку ниже — и мы включим тебя в рассылку форума.
        Так мы убеждаемся, что адрес действительно твой.
      </p>
      <p style="margin:0 0 22px">
        <a href="${url}" style="display:inline-block;background:#7B3FE4;color:#fff;text-decoration:none;font-weight:bold;font-size:15px;padding:12px 22px;border-radius:10px">
          Подтвердить подписку
        </a>
      </p>
      <p style="margin:0 0 16px;font-size:13px;line-height:1.5;color:#666">
        Если кнопка не открывается — скопируй ссылку в браузер:<br>
        <a href="${url}" style="color:#7B3FE4;word-break:break-all">${url}</a>
      </p>
      <p style="margin:16px 0 0;font-size:11.5px;color:#999">
        Если ты не подписывался(ась) на рассылку — просто проигнорируй это письмо,
        без подтверждения мы ничего не пришлём.
      </p>
    </div>
  `;
}
