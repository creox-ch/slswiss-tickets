...
/**
 * Ссылка подтверждения подписки для письма.
 *
 ...
 * как фишинг). На том домене стоит rewrite-прокси /newsletter/confirm → этот
 * API-роут (vercel.json сайта).
 *
...
export function confirmLink(sourceUrl, apiBase, token, allowedOriginsList) {
  ...
  return origin
    ? `${origin}/newsletter/confirm?token=${t}`
    : `${base}/api/forms/confirm?token=${t}`;
}
