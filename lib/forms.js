...
/** Почему вещь оказалась в очереди: пришла впервые или вернулась после правки. */
function queueCopy(reason) {
  return reason === 'edited'
    ? {
        lead: 'Продавец поправил уже опубликованную вещь — она ушла из каталога и ждёт повторной проверки.',
        line: 'правка опубликованной вещи',
      }
    : {
        lead: 'Продавец отправил вещь на проверку. Пока её не одобрить, в каталоге её нет.',
        line: 'первая проверка',
      };
}

/**
 * HTML письма модератору: вещь встала в очередь.
 *
 * Без него очередь молчит: продавец отправляет вещь и ждёт, а мы узнаём об этом,
 * только если случайно откроем /market/admin. Письмо служебное — уходит на
 * адреса из MARKET_ADMIN_EMAILS, не продавцу.
 *
 * sub = {itemLabel, sellerEmail, reason, queueUrl}
 */
export function renderMarketQueueHtml(sub) {
  const s = plainObject(sub);
  const copy = queueCopy(s.reason);
  const rows = reportRowsDesigned({
    Вещь: s.itemLabel || '—',
    Продавец: s.sellerEmail || '—',
    Повод: copy.line,
  });

  return emailShell({
    brand: 'market',
    title: 'Вещь на проверку — бренд-маркет Frankenplatz',
    heading: 'Вещь ждёт проверки',
    lead: copy.lead,
    blocks: rows,
    cta: s.queueUrl ? { href: String(s.queueUrl), label: 'Открыть очередь' } : null,
    footerNoteHtml: 'Служебное письмо модератора бренд-маркета. Продавцу оно не уходит.',
  });
}

/** Plain-text версия служебного письма об очереди. */
export function renderMarketQueueText(sub) {
  const s = plainObject(sub);
  const copy = queueCopy(s.reason);
  return [
    'FRANKENPLATZ MARKET · FASHION REBORN',
    ...
    `Продавец: ${s.sellerEmail || '—'}`,
    `Повод: ${copy.line}`,
    ...(s.queueUrl ? ['', `Очередь: ${String(s.queueUrl)}`] : []),
    '',
    '—',
    'Служебное письмо модератора бренд-маркета.',
  ].join('\n');
}
