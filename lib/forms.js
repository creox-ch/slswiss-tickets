 ...
 * и продолжало уходить покупателям. Вписать конкретный срок можно, когда он будет
 * решён, — тогда же поправить страницу маркета (репо frankenplatz).
...
export function renderMarketConfirmHtml(sub) {
  ...
  <tr><td class="fp-pad" style="padding:20px 34px 0">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
      <td style="padding:16px 18px;border-left:3px solid #E6B450;background:rgba(255,255,255,.035);border-radius:0 12px 12px 0">
        <span style="font-family:'Manrope',Arial,Helvetica,sans-serif;font-size:14px;line-height:1.6;color:#C3B7D4"><b style="color:#F3EEF9">Что дальше.</b> Ссылку на личный кабинет пришлём <b style="color:#F5C969">отдельным письмом</b>. Там загрузишь фото и цены, а мы соберём каталог. Офлайн-маркет — 27 сентября в Baden.</span>
      </td>
    </tr></table>
  </td></tr>`;
  return emailShell({
...
export function renderMarketConfirmText(sub) {
  ...
  return [
    ...
    `Оплачено: ${chf} CHF`,
    '',
    'Что дальше: ссылку на личный кабинет пришлём отдельным письмом.',
