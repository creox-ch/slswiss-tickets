...
/** Дефолтная копия (CC) уведомления — платформенному ассистенту. */
export const DEFAULT_NOTIFY_CC = 'assistant@creox.ch';

/**
 * Копия уведомления (CC). Переопределяется env FORMS_NOTIFY_CC:
...
export function renderOkaziyaHtml(sub) {
  ...
  return emailShell({
    ...
    blocks: submitted + replyNote,
    footerNoteHtml:
      'Ты получил(а) это письмо, потому что оставил(а) заявку на доске «Оказия» на frankenplatz.ch.',
  });
}
...
<table role="presentation" class="fp-wrap" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px;max-width:600px;background:#140A1F;border-radius:20px;overflow:hidden;border:1px solid rgba(255,255,255,.08);">
  ...
  <tr><td class="fp-pad" style="padding:22px 34px;border-bottom:1px solid rgba(255,255,255,.08)">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
      ...
      <td align="right" style="font-family:'Manrope',Arial,Helvetica,sans-serif;font-weight:700;font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:#E6B450">24–25 октября · Baden</td>
    </tr></table>
  </td></tr>
 ...
 * подтверждение участия: что куплено, сумма, что дальше (кабинет открывается 10.08).
 * Общая премиум-обёртка emailShell (как письма калькуляторов). sub = {name, description, amount}.
 */
export function renderMarketConfirmHtml(sub) {
  ...
  <tr><td class="fp-pad" style="padding:20px 34px 0">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
      <td style="padding:16px 18px;border-left:3px solid #E6B450;background:rgba(255,255,255,.035);border-radius:0 12px 12px 0">
        <span style="font-family:'Manrope',Arial,Helvetica,sans-serif;font-size:14px;line-height:1.6;color:#C3B7D4"><b style="color:#F3EEF9">Что дальше.</b> Личный кабинет откроется <b style="color:#F5C969">10 августа</b> — пришлём ссылку отдельным письмом. Там загрузишь фото и цены, а мы соберём каталог. Офлайн-маркет — 27 сентября в Baden.</span>
      </td>
    </tr></table>
  </td></tr>`;
  return emailShell({
    title: 'Frankenplatz Market — пакет оплачен',
    heading: `Ты в деле${name ? ', ' + name : ''} 🛍️`,
    lead: 'Оплата пакета получена. Спасибо! Ниже — детали заказа.',
    blocks: rows + next,
    footerNoteHtml:
      'Ты получил(а) это письмо, потому что оплатил(а) участие в бренд-маркете на frankenplatz.ch. Вопросы — <a href="mailto:info@frankenplatz.ch" style="color:#7A6C93">info@frankenplatz.ch</a>.',
  });
}
...
export function renderMarketConfirmText(sub) {
  ...
  return [
    ...
    `Оплачено: ${chf} CHF`,
    '',
    'Что дальше: личный кабинет откроется 10 августа — пришлём ссылку отдельным письмом.',
    'Там загрузишь фото и цены. Офлайн-маркет — 27 сентября в Baden.',
    '',
    '—',
    'Вопросы — просто ответь на это письмо или пиши info@frankenplatz.ch',
  ].join('\n');
