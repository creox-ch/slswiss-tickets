...
/**
 * Email из cookie запроса или null. Отдельная функция, чтобы роуты не
 * повторяли одно и то же и чтобы её можно было проверить юнит-тестом с
 * фейковым объектом запроса — `next/headers` в модуль тянуть незачем.
 */
export function sessionEmailFromRequest(req, secret, opts = {}) {
  if (!req || !req.cookies || typeof req.cookies.get !== 'function') return null;
  const cookie = req.cookies.get(SESSION_COOKIE);
  const value = cookie && typeof cookie === 'object' ? cookie.value : cookie;
  return verifySession(value, secret, opts);
}
