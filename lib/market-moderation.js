...
/**
 * Вкладки очереди: какие выборки модератору вообще доступны.
 *
 * Списком, а не «любой статус из адресной строки», по двум причинам. Первая:
 * `?status=draft` показывал чужие незаконченные черновики — вопреки решению
 * не показывать их никому, пока продавец не отправил вещь. Вторая: любое
 * произвольное значение рисовало пустую вкладку «zzz · 0», а API на нём падал
 * в 500 — то есть выглядело как «работы нет», а не как ошибка.
 */
export const MODERATION_TABS = ['pending', 'approved_market', 'approved_online', 'rejected', 'all'];

/** Значение фильтра из адреса → безопасная выборка. Мусор и draft → 'pending'. */
export function resolveTab(status) {
  const key = status === undefined || status === null ? '' : String(status);
  return MODERATION_TABS.includes(key) ? key : 'pending';
}
