 ...
 *
 * Считает функция в базе (`count_promo_uses`), а не фильтр отсюда. Путь по
 * вложенному JSON, написанный строкой в supabase-js, при ошибке не падает, а
 * молча отдаёт ноль — и код с лимитом «20 штук» работал бы бесконечно, о чём
 * не сказал бы ни один тест. В SQL то же выражение проверяемо, и оно одно на
 * весь проект (см. supabase-promo.sql). Проверено на боевой базе 20.08:
 * оплаченный заказ считается, pending — нет, регистр и пробелы нормализуются.
 */
async function countUses(code) {
  const { data, error } = await supabaseAdmin.rpc('count_promo_uses', { p_code: code });
