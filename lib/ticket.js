...
/**
 * Письмо-билет форума Frankenplatz (брендинг форума, детали заказа + QR).
 * Отдельно от SoiLüDi-билета: другой отправитель, другой текст, показывает
 * что куплено (день/категория/ланч) и сумму. QR/сканер — те же (общий стенд).
 */
export async function sendForumTicketEmail({ to, name, qrToken, description, amountRappen }) {
  const dataUrl = await buildQrDataUrl(qrToken);
  const base64 = dataUrl.split(',')[1];
  const qrUrl = `${SCAN_BASE}/api/qr?t=${encodeURIComponent(qrToken)}`;
  const safeName = name ? escapeHtml(name) : '';
  const safeDesc = escapeHtml(description || 'Билет на форум');
  const chf = (Number(amountRappen) / 100).toFixed(2);
  const from = process.env.FORUM_TICKET_FROM || 'Frankenplatz <noreply@frankenplatz.ch>';
  ...
  return resend().emails.send({
    ...
    html: `
      <div style="font-family:Arial,Helvetica,sans-serif;max-width:480px;margin:0 auto;color:#1a1a1a">
        ...
        <h2 style="margin:0 0 12px;font-size:22px;color:#231433">Билет у тебя${safeName ? ', ' + safeName : ''} 🎟️</h2>
        <p style="margin:0 0 14px;font-size:14px;line-height:1.5;color:#444">
          Оплата прошла. Покажи этот QR на входе — его отсканируют.
        </p>
        <table style="border-collapse:collapse;font-size:14px;width:100%;margin:0 0 6px">
          <tr><td style="padding:4px 12px 4px 0;color:#666">Билет</td><td style="padding:4px 0;color:#1a1a1a"><b>${safeDesc}</b></td></tr>
          <tr><td style="padding:4px 12px 4px 0;color:#666">Оплачено</td><td style="padding:4px 0;color:#1a1a1a"><b>${chf} CHF</b></td></tr>
        </table>
        <div style="text-align:center;margin:22px 0">
          <img src="${qrUrl}" alt="QR билета" width="240" height="240" style="width:240px;height:240px"/>
        </div>
        <p style="font-size:13px;color:#666">Код билета: ${qrToken}</p>
        <p style="font-size:12px;color:#999">Если QR не отображается — открой письмо в браузере. Вопросы — info@frankenplatz.ch.</p>
      </div>
    `,
    attachments: [
      {
